...
<div class="row g-3 mb-3">
    @php
        $cards = [
            ['title'=>'Total Users','value'=>($stats['users'] ?? 0),'icon'=>'bi-people'],
            ['title'=>'Pending Approvals','value'=>($stats['pending'] ?? 0),'icon'=>'bi-hourglass-split'],
            ['title'=>'Warehouses','value'=>($stats['warehouses'] ?? 0),'icon'=>'bi-building'],
            ['title'=>'Medicines','value'=>($stats['medicines'] ?? 0),'icon'=>'bi-capsule'],
            ['title'=>'Categories','value'=>($stats['categories'] ?? 0),'icon'=>'bi-grid'],
            ['title'=>'Banners','value'=>($stats['banners'] ?? 0),'icon'=>'bi-image'],
            ['title'=>'Orders','value'=>($stats['orders'] ?? 0),'icon'=>'bi-receipt'],
        ];
    @endphp
                    ...
                    <table class="table align-middle mb-0">
                        ...
                        @forelse($recentOrders as $o)
                            <tr>
                                ...
                                <td>{{ data_get($o, 'user.name', '-') }}</td>
                                <td>{{ data_get($o, 'warehouse.name', '-') }}</td>
                                <td><span class="badge text-bg-primary-subtle text-primary-emphasis">{{ $o->status }}</span></td>
                                <td class="text-end fw-bold">{{ number_format((float)($o->total ?? 0),2) }}</td>
                            </tr>
                        @empty
